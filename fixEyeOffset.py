	...
	for row in csv_reader:
		if row != []:
			if row[1] == "Condition":
				...
				if figure in shape and "separated" not in shape:
					x_eye = float(row[2])
					y_eye = float(row[3])
		...
		for width in widths: 
			for figure in shapes:
				...
				for filename in os.listdir('offsetClick/'):
					...
					q("offsetClick/" + filename, overall, items, counter, amplitudes, counterAmps, x_eyes, y_eyes, x_hits, y_hits, figure, width, amp, x_offsets, y_offsets)
